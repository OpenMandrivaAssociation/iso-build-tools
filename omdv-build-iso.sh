...
# This lot allows the chroot to be retained when debugging local builds
# ultimately slight restructuring will allow it to be used for users to 
# create their own custom builds
set -x
if [ -z "$ABF" ] && [ -s $OURDIR/ROOT_NME ] && [ "$DEBUG" == "nodebug" ]; then
    ROOTNAME="`mktemp -d /tmp/liverootXXXXXX`"
    [ -z "$ROOTNAME" ] && ROOTNAME=/tmp/liveroot.$$
    echo $ROOTNAME >$OURDIR/ROOT_NME
else
    if [ -z "$ABF" ] && [ -s $OURDIR/ROOT_NME ] && [ "$DEBUG" == "debug" ]; then
#	$ROOTNAME=`tail ROOT_NME`
	[ -z "$ROOTNAME" ] && ROOTNAME=`tail ROOT_NME`
    else
        ROOTNAME="`mktemp -d /tmp/liverootXXXXXX`"
	[ -z "$ROOTNAME" ] && ROOTNAME=/tmp/liveroot.$$
	echo $ROOTNAME >$OURDIR/ROOT_NME
...
keepChroot(){
	if [ -z "$ABF" ] && [ "${DEBUG,,}" == "debug" ]; then
	    if [ -s $OURDIR/CHRT_NME ]; then
		$ROOTNAME=`cat $OURDIR/ROOT_NME`
	    else
		echo "Need to run once to create the chroot"
	    fi
	fi
}
...
getPkgList() {
    ...
    # or for developing specialist builds. We also need to check if the user updated it
...
# If debug is set but $CHROOTDIR has only just been created it will be empty so we must still
# build it the first time.

createChroot() {
	if [ -s $OURDIR/ROOT_NME ] && [ "${DEBUG,,}" == "debug" ] && [ -d $CHROOTDIR/BASE/lib ]; then
	...
	# Only update media when it's out of date or the update flag is set
#WORK IN PROGRESS
#	if [ -s $OURDIR/ROOT_NME ] && [ "${DEBUG,,}" == "debug" ]; then
...
createInitrd() {
	...
	if [ ! -d "$1"/usr/lib/dracut/modules.d/90liveiso ]; then
	    ...
	    #BUG
	    # Before we can do anything we must fix the perms on /usr/lib/ so we can write the stuff that follows.
#		I don't understand how we are able to write to a read-only directory
#	    $SUDO chmod 0755 "$1"/usr/lib
...
#		$SUDO cp -f "$1"/boot/grub2/splash.xpm.gz "$2"/EFI/BOOT/splash.xpm.gz
		$SUDO cp -a -f "$1"/boot/grub2/themes/* "$2"/EFI/BOOT/themes/OpenMandriva/
...
EOF
	...
	# TPG DO WE REALLY NEED TO DO THIS. IT'S ALREADY BEEN DONE WHEN THE CHROOT WAS CREATED
	# AS THE CHROOT IS NOT PERISTANT IT DOES SEEM UNECESSARY. THE 32 BIT REPOS SHOULD BE ENOUGH
	#
    if [ "$DEBUG" == "nodebug" ]; then

...
LABEL boot
	...
	APPEND initrd=/isolinux/liveinitrd.img rootfstype=auto ro rd.luks=0 rd.lvm=0 rd.md=0 rd.dm=0 rd.live.image quiet rhgb vga=788 splash=silent logo.nologo root=live:/dev/disk/by-label/$LABEL locale.lang=en_US vconsole.keymap=us

LABEL install
	MENU LABEL Install OpenMandriva Lx
	LINUX /isolinux/vmlinuz0
	INITRD /isolinux/liveinitrd.img
	APPEND initrd=/isolinux/liveinitrd.img rootfstype=auto ro rd.luks=0 rd.lvm=0 rd.md=0 rd.dm=0 rd.live.image quiet rhgb vga=788 splash=silent logo.nologo root=live:/dev/disk/by-label/$LABEL locale.lang=en_US vconsole.keymap=us install

LABEL vesa
	MENU LABEL Boot OpenMandriva Lx in safe mode
	LINUX /isolinux/vmlinuz0
	INITRD /isolinux/liveinitrd.img
	APPEND initrd=/isolinux/liveinitrd.img rootfstype=auto ro rd.luks=0 rd.lvm=0 rd.md=0 rd.dm=0 rd.live.image xdriver=vesa nomodeset plymouth.enable=0 vga=792 install root=live:/dev/disk/by-label/$LABEL locale.lang=en_EN vconsole.keymap=en
